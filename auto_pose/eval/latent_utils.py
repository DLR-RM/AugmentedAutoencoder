...
def shape_retrieval(sess, args_latent, dataset, codebook):
    ...
    obj_names = eval(args_latent.get('ShapeRetrieval','obj_names'))
    ...
    H_AE, W_AE = dataset.shape[:2]
    t_z = float(dataset._kw['radius'])
    model_paths = eval(str(dataset._kw['model_path']))
    
    codebook_list = [codebook._get_codebook_name(mp) for mp in model_paths]
    print(codebook_list)
    ...
    joint_views = []
    joint_views_back = []
    for obj_i in range(0, num_obj):
        obj_id = obj_i + 1
        target_views =[]

        # test_img_crops = eval_utils.get_sixd_gt_train_crops(obj_id, (H_AE, W_AE), pad_factor=pad_factor, dataset='tless', cam_type='primesense')
        test_img_crops = eval_utils.get_moped_gt_crops(dataset, obj_names[obj_i], (H_AE, W_AE), num_views, pad_factor=1.2)

        for j in range(num_views):
            #
            #target_view = test_img_crops[np.random.randint(len(test_img_crops))][obj_id][0]
            target_view = test_img_crops[j]

            # random in-plane rotation, not necessary
            for i in range(5):
                rot_angle = np.random.rand()*360
                cent = int(target_view.shape[0]/2)
                M = cv2.getRotationMatrix2D((cent,cent),rot_angle,1)
                target_views.append(cv2.warpAffine(target_view,M,target_view.shape[:2]))
            
            ################### random synthetic views as input
            # 
            
            # target_view, target_view_dep= dataset.renderer.render(obj_id=obj_i,
            #                                                 W=render_dims[0],
            #                                                 H=render_dims[1],
            #                                                 K=K.copy(),
            #                                                 R=random_R,
            ...
            # ys, xs = np.nonzero(target_view_dep > 0)
            # target_bb = view_sampler.calc_2d_bbox(xs, ys, render_dims)
            # target_view = dataset.extract_square_patch(target_view, target_bb, pad_factor)
            ################### 

            ################### Embedding views
            # target_views, _ = dataset.render_embedding_image_batch(1000,1010)
            ################### 

            # cv2.imwrite('target_view_{}.png'.format(j), target_view)

            # target_views.append(target_view)
        retr_dict = codebook.retrieve_nearest_shape(sess, np.array(target_views),top_n=3,dist_matching='backwards_mapping')
        
        mean_cosines = retr_dict['mean_cosines']
        mean_cosines_back = retr_dict['mean_cosines_back']
        mean_cosines_mixed = retr_dict['mean_cosines_mixed']
        nearest_models = retr_dict['nearest_models']
        nearest_models_back = retr_dict['nearest_models_back']
        nearest_models_mixed = retr_dict['nearest_models_mixed']

        import copy
        shape_scores[obj_names[obj_i]] = copy.deepcopy(mean_cosines)
        nearest_shapes[obj_names[obj_i]] = nearest_models[0]

        ##########
        
        for selected_nearest_models in [nearest_models, nearest_models_back, nearest_models_mixed]:
            codebook_views = []

            for nearest_sh in selected_nearest_models:
                cb_idx = codebook_list.index(nearest_sh)

                R = codebook.nearest_rotation(sess, target_view, nearest_sh)

                codebook_view, codebook_view_dep = dataset.renderer.render(obj_id=cb_idx,
                                                                W=render_dims[0],
                                                                H=render_dims[1],
                                                                K=K.copy(),
                                                                R=np.array(R).squeeze().reshape(3,3),
                                                                t=np.array([0,0,t_z]),
                                                                near=clip_near,
                                                                far=clip_far,
                                                                random_light=False)

                ys, xs = np.nonzero(codebook_view_dep > 0)
                target_bb = view_sampler.calc_2d_bbox(xs, ys, render_dims)
                view = dataset.extract_square_patch(codebook_view, target_bb, pad_factor)
                view = cv2.putText(view, '{:3.2f}'.format(mean_cosines[nearest_sh]), (80, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                view = cv2.putText(view, '{:3.2f}'.format(mean_cosines_back[nearest_sh]), (80, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                view = cv2.putText(view, '{:3.2f}'.format(mean_cosines_mixed[nearest_sh]), (80, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
                codebook_views.append(view)
            joint_views.append(np.hstack((target_view, codebook_views[0], codebook_views[1], codebook_views[2])))
        ##########


        print('nearest model is {} with average cosine similarity {}'.format(nearest_models[0], mean_cosines[nearest_models[0]]))

    with open('shape_scores_all.pickle'.format(obj_names), 'w') as f:
        pickle.dump(shape_scores,f)

    nearest_shape_views = u.tiles(np.array(joint_views), 32, 3)
    cv2.imwrite('nearest_shape_views.png', nearest_shape_views)

    fig, axs = plt.subplots(4,4)
    for i,key in enumerate(sorted(shape_scores.keys())):
        print(axs.shape)
        axs[i//4,i%4].set_title(str(key) + '\n Nearest: ' + str(nearest_shapes[key]))
        sorted_bins = [float(shape_scores[key][k]) for k in sorted(shape_scores[key].keys())]
        barlist=axs[i//4,i%4].bar(np.arange(1, len(sorted_bins)+1),sorted_bins)
        # barlist[i].set_facecolor('lime')

    plt.savefig('shape_scores.pdf')
    plt.show()
