...
def map_decorator(name, p_offset=0.5):
	def real_decorator(fn):
		def fn_wrapper(*argv, **kwargs):
			...
			with tf.variable_scope(name):
				...
				### Probablilites
				new_shape = values.get_shape().as_list()
				prob = tf.random_uniform([new_shape[0]], minval = 0.0, maxval = 1.0, dtype = tf.float32)
				masks = tf.where(tf.less(prob, p_offset), tf.ones_like(prob), tf.zeros_like(prob))
				masks = tf.tile(masks[:, tf.newaxis, tf.newaxis, tf.newaxis], [1, new_shape[1], new_shape[2], new_shape[3]])
				new_kwargs['aug_mask'] = masks
...
def reshape_and_pad(in_tensor, zoom_value, mode, aug_mask=None):
...
def crop_and_reshape(in_tensor, zoom_value, aug_mask=None):
...
def zoom_image_object(in_tensor, zoom_range, padding_mode = 'CONSTANT', aug_mask=None):
	""" Zoom image objects 
...
def translate_im_obj(in_tensor, trans_range, aug_mask=None):
...
def zoom_and_translate(in_tensor, zoom_range, trans_range, aug_mask=None):
...
def in_plane_rotation(in_tensor, angle = None, aug_mask=None):
...
def add_black_patches(in_tensor, max_area_cov = 0.25, max_patch_nb = 5, aug_mask=None):

	def square_patch(in_tensor, aug_mask, max_coverage = 0.05):
		...
		mask = tf.contrib.image.translate(mask, 
					...
					interpolation = 'NEAREST') * aug_mask

		mask = in_plane_rotation(mask)

		return tf.multiply(in_tensor, 1.0 - mask)

	def circle_patch(in_tensor, aug_mask, max_coverage = 0.05):
		"""
	...
	for i in range(max_patch_nb):
		patch_prob = tf.random_uniform([modified_tensor.get_shape().as_list()[0]], minval = 0.0, maxval = 1.0, dtype = tf.float32)
		
		square_prob = tf.random_uniform([modified_tensor.get_shape().as_list()[0]], minval = 0.0, maxval = 1.0, dtype = tf.float32)
		square_mask = tf.where(tf.logical_and(tf.less(patch_prob, 0.5), tf.less(square_prob, 0.5)), 
			tf.ones_like(patch_prob), 
			tf.zeros_like(patch_prob))
		#square_mask = tf.where(tf.less(square_prob, 0.5), square_mask, tf.zeros_like(patch_prob))

		circle_mask = tf.where(tf.logical_and(tf.less(patch_prob, 0.5), tf.greater_equal(square_prob, 0.5)), 
			tf.ones_like(patch_prob), 
			tf.zeros_like(patch_prob))

		square_mask = tf.tile(square_mask[:, tf.newaxis, tf.newaxis, tf.newaxis], [1]+modified_tensor.get_shape().as_list()[1:])
		circle_mask = tf.tile(circle_mask[:, tf.newaxis, tf.newaxis, tf.newaxis], [1]+modified_tensor.get_shape().as_list()[1:])

		modified_tensor = square_patch(modified_tensor, aug_mask*square_mask, max_coverage)
		modified_tensor = circle_patch(modified_tensor, aug_mask*circle_mask, max_coverage)
...
def gaussian_noise(in_tensor, stddev = 0.01, aug_mask=None):
	...
	noise = tf.random_normal(in_tensor.get_shape().as_list(), mean = 0.0, stddev = stddev, dtype = tf.float32)
	return tf.maximum(tf.minimum(in_tensor+tf.multiply(noise, aug_mask), 1.0), 0.0)
...
def random_brightness(in_tensor, max_offset, aug_mask=None):
	
	def aug_all_color_channels(in_tensor, aug_mask, max_offset):
		...
		return in_tensor+(brightness_offset*aug_mask)

	def aug_one_color_channel(in_tensor, aug_mask, max_offset):
		shape = in_tensor.get_shape().as_list()
		#print(shape)
		brightness_offset = tf.random_uniform([shape[0]], minval = -max_offset, maxval = max_offset, dtype = tf.float32)
		...
		return in_tensor + (offset*aug_mask)
	...
	all_channel_prob = tf.random_uniform([shape[0]], minval = 0.0, maxval = 1.0, dtype = tf.float32)
	mask = tf.where(tf.less(all_channel_prob, 0.5), tf.ones_like(all_channel_prob), tf.zeros_like(all_channel_prob))
	mask = tf.tile(mask[:, tf.newaxis, tf.newaxis, tf.newaxis], [1]+shape[1:])
	
	aug_tensor = aug_all_color_channels(in_tensor, mask, max_offset)

	one_channel_prob = tf.random_uniform([in_tensor.get_shape().as_list()[0]], minval = 0.0, maxval = 1.0, dtype = tf.float32)
	mask = tf.where(tf.less(one_channel_prob, 0.5), tf.ones_like(one_channel_prob), tf.zeros_like(one_channel_prob))
	mask = tf.tile(mask[:, tf.newaxis, tf.newaxis, tf.newaxis], [1]+shape[1:])
	
	aug_tensor = aug_one_color_channel(in_tensor, mask, max_offset)
...
def add_background_and_brightness_augment(in_tensor, bg_images, max_offset, bg_color=0, aug_mask=None):
	if bg_color==0:
...
def gaussian_blur(in_tensor, size = 5, aug_mask=None):
...
def gamma_normalization(in_tensor, f = [0.5, 2.2], aug_mask=None):
	'''
	...
	'''
	def _normalize(in_tensor, aug_mask, f):
	...
	all_channel_prob = tf.random_uniform([in_tensor.get_shape().as_list()[0]], minval = 0.0, maxval = 1.0, dtype = tf.float32)
	mask = tf.where(tf.less(all_channel_prob, 0.5), tf.ones_like(all_channel_prob), tf.zeros_like(all_channel_prob))
	mask = tf.tile(mask[:, tf.newaxis, tf.newaxis, tf.newaxis], [1]+in_tensor.get_shape().as_list()[1:])
	
	in_tensor = _normalize(in_tensor, mask*aug_mask, f)

	mask = tf.where(tf.greater_equal(all_channel_prob, 0.5), tf.ones_like(all_channel_prob), tf.zeros_like(all_channel_prob))
	mask = tf.tile(mask[:, tf.newaxis, tf.newaxis, tf.newaxis], [1]+in_tensor.get_shape().as_list()[1:])

	return _one_channel(in_tensor, mask*aug_mask, f)

@map_decorator('contrast_normalization')
def contrast_normalization(in_tensor, f = [0.5, 2.0], aug_mask=None):
	'''
	...
	def _normalize(in_tensor, aug_mask, f):
		...
		new_tensor = in_tensor + (in_tensor - means) * factor * aug_mask
		return tf.clip_by_value(new_tensor, 0.0, 1.0)
		...
		for i, channel in enumerate(channels):
			new_channels.append(tf.where(tf.equal(i, c),
				channel + (channel - tf.tile(tf.reduce_mean(channel, axis = [1, 2])[:, tf.newaxis, tf.newaxis], 
					[1, shape[1], shape[2]]))*factor*aug_mask,
				channel))
...
def multiply_brightness(in_tensor, factor_range = [0.6, 1.4], aug_mask=None):
	'''
	...
	all_channel_prob = tf.random_uniform([in_tensor.get_shape().as_list()[0]], minval = 0.0, maxval = 1.0, dtype = tf.float32)
	mask = tf.where(tf.less(all_channel_prob, 0.5), tf.ones_like(all_channel_prob), tf.zeros_like(all_channel_prob))
	mask = tf.tile(mask[:, tf.newaxis, tf.newaxis], [1]+shape[1:3])
	
	in_tensor = _all_channels(in_tensor, tf.where(tf.equal(mask*aug_mask, 1), factor, tf.ones_like(factor)))

	mask = tf.where(tf.greater_equal(all_channel_prob, 0.5), tf.ones_like(all_channel_prob), tf.zeros_like(all_channel_prob))
	mask = tf.tile(mask[:, tf.newaxis, tf.newaxis], [1]+shape[1:3])

	return _one_channel(in_tensor, tf.where(tf.equal(mask*aug_mask, 1), factor, tf.ones_like(factor)))

@map_decorator('invert_color')
def invert_color(in_tensor, aug_mask=None):

	def _all_channels(in_tensor, aug_mask):
		return tf.where(tf.equal(aug_mask, 1.0), 1.0 - in_tensor, in_tensor)

	def _one_channel(in_tensor, aug_mask):
		channel = tf.random_uniform([], minval = 0, maxval = 3, dtype = tf.int32)
		c = in_tensor.get_shape().as_list()[3]
		channels = tf.unstack(in_tensor, c, 3)
		new_channels = []
		...
		aug_mask = tf.reduce_mean(aug_mask, 3)
		#print(aug_mask.get_shape().as_list())

		for i, tensor_channel in enumerate(channels):
			new_channels.append(tf.where(tf.equal(i, channel),
				tf.where(tf.equal(aug_mask, 1.0), 1.0 - tensor_channel, tensor_channel),
				tensor_channel))
	...
	in_tensor = _all_channels(in_tensor, aug_mask*mask)

	mask = tf.where(tf.greater_equal(all_channel_prob, 0.5), tf.ones_like(all_channel_prob), tf.zeros_like(all_channel_prob))
	mask = tf.tile(mask[:, tf.newaxis, tf.newaxis, tf.newaxis], [1]+in_tensor.get_shape().as_list()[1:])

	return _one_channel(in_tensor, aug_mask*mask)
...
def main():
	...
	#out_tensor = contrast_normalization(in_image, [0.5, 2.2])
	#out_tensor = multiply_brightness(in_image, [0.6, 1.4])
	#out_tensor = gamma_normalization(in_image, [0.5, 2.2])
	#out_tensor = random_brightness(in_image, 0.5)
	#out_tensor = gaussian_noise(in_image, 0.01)
	out_tensor = add_black_patches(in_image, 0.25, 5)
